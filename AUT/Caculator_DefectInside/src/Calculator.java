...
//		m_inputNum = new BigInteger("0");
	...
	public void SEActionPerformed(ActionEvent evt) {
		...
		m_memory = Integer.parseInt(content.substring(0, content.length() - 1));
		m_inputNum = new BigInteger("0");
		setMemoryLable();
	}
	...
	public void MPlusActionPerformed(ActionEvent evt) {
		String content = m_ResultText.getText();
		int addnumber = Integer.parseInt(content.substring(0, content.length() - 1) );
		m_memory = m_memory + addnumber;
		m_inputNum = new BigInteger("0");
		setMemoryLable();
	}
...
// defect, else下的不會再有動作, logic錯誤.. S.K.Wen
//		else
...
// Defect 永遠會 setText 為 M , S.K.Wen
//		if (m_memory != 0)
...
//			m_MemLabel.setText("");
	...
	private BigInteger compute(BigInteger righthand, BigInteger lefthand) {
		if (m_op == 1)
