...
public class LackEncapsulationSmellVisitor extends MacroRefactorVisitor {
	...
	private final String BS_LACK_ENCAPSULATION = "Lack Encapsulation";
	
	@Override
	public void visit(ComponentEventNode node) {
		super.visit(node);
		AbstractMacroNode parent = node;
		MacroComponentNode mcompnode = null;
		while(parent != null && (parent.getParent() instanceof MacroComponentNode) == false)
			parent = parent.getParent();
		if(parent == null)
			return;
		mcompnode = (MacroComponentNode)parent.getParent();
		
		if(node.getComponentPath().startsWith(mcompnode.getPath().toString()) == false) {
			...
			m_item = new BadSmellItem(BS_LACK_ENCAPSULATION, node.getPath().toString(), IMPORTANCE_YELLOW);
			m_item.addNode(node);
