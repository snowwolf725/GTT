...
import gtt.eventmodel.swing.SwingTag;
...
public class WebDescriptorReader implements IDescriptorReader {
	WebModel theModel = null;
	String cur_Scope = "";
	String m_File;
	
	public WebDescriptorReader(String desc_file) {
		m_File = desc_file;
	}
	
	public void setModel(IEventModel model) {
		theModel = (WebModel)model;
	}
	...
	public boolean read() {
		...
		try {
			...
			if (doc.getChildNodes().item(0).getNodeName().compareTo(SwingTag.ROOT_NAME) != 0) {
	...
	protected void dispatch(Node node) {
		 ...
		 */
		if (name.compareTo(WebTag.SCOPE) == 0) {
		 ...
		 */
		if (name.compareTo(WebTag.COMPONENT_SET) == 0) {
		 ...
		 * <Events> </Events>
		 */
		if (name.compareTo(WebTag.EVENT_SET) == 0) {
	...
	private void branchNode(Element ele) {
		...
		if( cur_Scope.length()==0 )
			cur_Scope = ele.getAttribute(WebTag.NAME);
		else {
			cur_Scope += "." + ele.getAttribute(WebTag.NAME);
	...
	private void singleComponentNode(Element ele) {
		// find a Html component information

		String type = ele.getAttribute(WebTag.COMPONENT_CLASS_TYPE);
		
		/* 增加一個新的 HTMLComponent */
		HTMLComponent c = HTMLComponent.createComponent(type, "", "", "", "", 0, 0);
		theModel.addComponent( c );
		/* 記錄 HTMLComponent　的package資訊 */
	...
	private void singleEventNode(Element ele) {
		...
		String name = ele.getAttribute(WebTag.NAME);	
		String data = ele.getAttribute(WebTag.DATA);
