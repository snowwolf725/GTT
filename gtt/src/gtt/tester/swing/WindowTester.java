...
public class WindowTester extends JComponentTester {
	...
	public boolean fireEvent(IEvent info, Object comp) {
		...
		WindowOperator _compEvent = new WindowOperator((Window) comp);
		...
		if (eid == SwingTesterTag.CLOSE) {
			_compEvent.requestCloseAndThenHide();
			return true;
		}
		if (eid == SwingTesterTag.MOVE) {
			int xpos = Integer.parseInt(info.getArguments().getValue("X"));
			int ypos = Integer.parseInt(info.getArguments().getValue("Y"));
			_compEvent.move(xpos, ypos);
			return true;
		}
