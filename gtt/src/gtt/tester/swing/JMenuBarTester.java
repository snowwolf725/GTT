...
public class JMenuBarTester extends JComponentTester {
	...
	public boolean fireEvent(IEvent info, Object comp) {
		...
		JMenuBarOperator _compEvent = new JMenuBarOperator((JMenuBar) comp);
		int eid = info.getEventId();

		if (eid == SwingTesterTag.CLOSE_SUBMENUS) {
			_compEvent.closeSubmenus();
			return true;
		}
		if (eid == SwingTesterTag.PUSH_MENU) {
			_compEvent.pushMenu(info.getArguments().getValue("Path"), "|");
			return true;
		}
		if (eid == SwingTesterTag.PUSH_MENU_NO_BLOCK) {
			_compEvent.pushMenuNoBlock(info.getArguments().getValue("Path"),
					"|");
			return true;
		}
