...
public class AddSourceFolderNodeDialog extends TitleAreaDialog{
	private String _nodeName;
	private Text _text;
	private SourceFolderNode _sourceFolderNode;
	private ProjectNode _projectNode;
	private TreeViewer _viewer;
	
	
	public AddSourceFolderNodeDialog(Shell parentShell) {
		super(parentShell);
		// TODO Auto-generated constructor stub
	}
	
	protected Control createDialogArea(Composite parent) {
		this.getShell().setText( "Type in the name of SourceFolder" );
		this.setTitle("Insert Suggestion");
		this.setMessage("Insert all suggestions to Test Case Project");
		...
		label.setText("SourceFolder Name:");
...
//				String folder = _projectNode.addNewSourceFolder(_text.getText());
				manager.addJavaSourceFolder(prj, _text.getText());
				
				TestScriptNodeFinder nodefinder = new TestScriptNodeFinder();
				SourceFolderNode srcfolder = (SourceFolderNode)nodefinder.findNode(new Path("/"+_projectNode.getName()+"/"+_text.getText()));
				BaseNode[] baseNodes = _sourceFolderNode.getChildren();
				
				for(int index = 0; index < baseNodes.length; index++) {
					String filename = ((TestCaseNode)baseNodes[index]).getName();
					...
					if (result == false) {
						...
						BaseNode[] methodNodes = ((TestCaseNode)baseNodes[index]).getChildren();
	...
	public void setSourceFolderNode(SourceFolderNode sourceFolderNode) {
		_sourceFolderNode = sourceFolderNode;
	}
