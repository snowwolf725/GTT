 ...
 * 
 */
public class AddOracleNodeVisitor_Event implements ITestScriptVisitor {
	...
	public AddOracleNodeVisitor_Event(Combo _com_strategy, Tree _tree){
		m_tree = _tree;
	...
	public void visit(FolderNode node) {
		...
		while(ite.hasNext()){
			EventNode eventnode = (EventNode)ite.next();
			if(eventnode.getParent() != node) continue;
			if( m_com_strategy.getSelectionIndex() == 0){
				...
				for(int i = 0 ; i< m_tree.getItemCount() ; i++){
					...
					eventnode.getParent().add(oraclenode.clone() , eventnode.getParent().indexOf(eventnode)+1);
				...
				for(int i = 0 ; i< m_tree.getItemCount() ; i++){
					TreeItem treeitem = m_tree.getItem(i);
					OracleNode oraclenode = (OracleNode)treeitem.getData();
					eventnode.getParent().add(oraclenode.clone() , eventnode.getParent().indexOf(eventnode));
					oraclenode.setParent(node);
				}
			}
		}
